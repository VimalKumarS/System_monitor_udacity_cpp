...
/*
void NCursesDisplay::DisplayProcesses_test(std::vector<Process>& processes,int n) {

    int const num_processes = int(processes.size()) > n ? n : processes.size();
    for (int i = 0; i < num_processes; ++i) {
        std::cout <<  to_string(processes[i].Pid()).c_str()<< std::endl;
        std::cout <<  processes[i].User().c_str() << std::endl;
        float cpu = processes[i].CpuUtilization() * 100;
        std::cout << to_string(cpu).substr(0, 4).c_str()<< std::endl;
        std::cout << processes[i].Ram().c_str()<< std::endl;;
        std::cout <<
                  Format::ElapsedTime(processes[i].UpTime()).c_str()<< std::endl;
        std::cout <<
                  processes[i].Command().c_str()<< std::endl;
    }
}
...
void NCursesDisplay::Display_test(System& system, int n) {
    std::cout << "start";
//    while (1) {
        DisplaySystem_test(system);
        DisplayProcesses_test(system.Processes(), n);
        refresh();
